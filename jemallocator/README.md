...
The project is also published as `jemallocator` for historical reasons. The two crates are the same except names. For new projects, it's recommended to use `linera-xxx` versions instead.
...
* `linera-jemalloc-sys`: builds and links against `jemalloc` exposing raw C bindings to it.
* `linera-jemallocator`: provides the `Jemalloc` type which implements the
  `GlobalAlloc` and `Alloc` traits. 
* `linera-jemalloc-ctl`: high-level wrapper over `jemalloc`'s control and introspection
...
To use `linera-jemallocator` add it as a dependency:
...
linera-jemallocator = "0.6"
```

To set `linera_jemallocator::Jemalloc` as the global allocator add this to your project:
...
* `run`: do `linera-jemallocator` and `linera-jemalloc-sys` tests pass on the target?
...
* The `linera-jemallocator` crate re-exports the [features of the `linera-jemalloc-sys`
dependency](https://github.com/linera-io/jemallocator/blob/master/jemalloc-sys/README.md#features).
...
for inclusion in `linera-jemallocator` by you, as defined in the Apache-2.0 license,
...
[ci]: https://github.com/linera-io/jemallocator/actions/workflows/main.yml/badge.svg
[github actions]: https://github.com/linera-io/jemallocator/actions
